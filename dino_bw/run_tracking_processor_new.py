...
def main(
    ...
    cache_patch_size = cache_data.get('patch_size', 16)
    cache_image_size = cache_data.get('image_size', 768)
    ...
    for i, frame_idx in enumerate(frame_indices):
        ...
        frame_data = cache_data['frames'][frame_idx]
...
if __name__ == "__main__":
    ...
    data_folder = Path("/path/to/bw2508_dataset")
    cache_path = Path("/path/to/tracking_cache_img_1024_patch_16.pkl")
    debugging_folder = Path("/path/to/debugging_output")
    
    # Create debugging folder if it doesn't exist
    debugging_folder.mkdir(exist_ok=True)
    
    # Run with custom frame indices
    frame_indices = [200, 205, 210, 215]  # Example: 4 consecutive frames
